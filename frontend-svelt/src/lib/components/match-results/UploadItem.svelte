<script lang="ts">
	...
	let { file, isUploaded, onRemove }: Props = $props();
	let fileType = $derived(file.name.split('.').pop());
</script>

<div class="w-full">
	<svg
		class="inline-block h-6 w-6 text-gray-800"
		aria-hidden="true"
	...
	<span class="overl inline-block">
		{file.name}
	</span>
	<button
		class="inline-block rounded-full hover:bg-red-100"
		title="Remove file"
		onclick={() => onRemove(file)}
	>
		{#if isUploaded}
			<svg
				class="h-6 w-6 text-green-500 text-green-800"
				aria-hidden="true"
				xmlns="http://www.w3.org/2000/svg"
				width="24"
				height="24"
