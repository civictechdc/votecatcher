...
| G7: Matching Service Refactor | 🔄 In progress | 12 pass (5 adapter + 6 spec-driven + 1 approval) | **G7.0 DONE:** `flatten_voter_data` in `app/matching/voter_data_adapter.py` — flattens voter JSON blobs to flat dict keyed by voter_reg_field.id, category→blob mapping. **G7.1 DONE:** Baseline approval test captured in `test_matching_regression_approval.py` — 5 OCR inputs × 4 voters score matrix. **G7.2+G7.3 DONE:** `calculate_spec_driven_similarity` added to MatchingService — uses spec weights, render_template, flatten_voter_data. field_scores keyed by ballot_field.id. **G7.3a IN PROGRESS:** Added `pre_filter_field_id` to RegionFieldSpecConfig + dc.json5. Added `pre_filter_voters_with_spec` method. **⚠️ UNCOMMITTED:** G7.3a code in working tree — needs tests + dc.json5 approval snapshot re-approve + `_dc_spec()` test helper update. **Remaining:** G7.3a tests, G7.4 re-run baseline, G7.5 cleanup, G7.6 doc full_name defaults, G7.7 worker.py consolidation, G7.8 matching-process.md, G7.9 C4 update. 1021 total tests pass (was 1009 at G6 exit). |
