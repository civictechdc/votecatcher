...
def get_ocr_provider_config(
	...
	if config is None:
		raise ValueError("OCR provider config not set.")
...
def get_current_ocr_client() -> BatchOcrClient:
	...
	if _current_ocr_client is None:
		raise ValueError("OCR Batch client was not initialised.")
...
async def observe_batch_job_status(job_id: str) -> AsyncGenerator[BatchJobStatus]:
	try:
		...
		logger.error("Job failed with exception", exception=e)
...
) -> AsyncGenerator[OcrResultItem]:
	client: BatchOcrClient = _create_batch_ocr_client(config)
	result: AsyncGenerator[OcrResultItem] = client.get_ocr_results(job_id)
	return result
