...
class BatchOcrHandler:
    ...
    async def _update_task_status(self, ocr_job: OcrJobStatus) -> MatchingTask:
        task_update: UpdateMatchingTask = UpdateMatchingTask(
            ...
            updated_at=(
                ocr_job.updated_at if ocr_job.updated_at else datetime.now(timezone.utc)
            ),
            ocr_result_data=(
        ...
        self._active_ocr_jobs[ocr_job.ocr_job_id] = (
        ...
        if (
            ...
            logger.debug(f"Ocr Job is terminal in update")
        ...
        while True:
            try:
                ...
                if (
                    ...
                    logger.debug(f"Job has reached terminal in ocr batch loop")
                ...
                logger.error(f"OCR poller error", job_id=job_id, error=str(exc))
                task_update: UpdateMatchingTask = UpdateMatchingTask(
