...
from app.data.database.local.demo_ocr_results import OcrResultEntity
...
def create_demo_records():
    with Session(engine) as session:
        ...
        column_specs: list[MatchResultColumns] = [
            MatchResultColumns(
                name="Name",
                data_type="str",
                position_index=0,
...
class LocalRegisteredVoterRepository:
    ...
    async def save_registered_voter_list(
        ...
        assert voter_data is not None, f"Voter data frame is unexpectedly null"
